...
import { FaClipboardCheck, FaSearch, FaEye, FaEdit, FaTrash } from 'react-icons/fa'
...
export default function VerifikasiRMPage() {
  const [records] = useState(initialData)
  const [searchTerm, setSearchTerm] = useState('')

  const filteredRecords = records.filter(record =>
    record.patientName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    record.medicalRecordNumber.toLowerCase().includes(searchTerm.toLowerCase())
  )
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 transition-all duration-500 bg-gradient-to-br from-gray-100 via-white to-gray-50 dark:from-zinc-950 dark:via-neutral-900 dark:to-zinc-800 text-gray-900 dark:text-gray-100">
      <h1 className="text-2xl md:text-3xl font-bold mb-6 tracking-wide flex items-center gap-3">
        ...
        <span className="truncate">Verifikasi Rekam Medis</span>
      </h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-zinc-700 text-left">
                ...
                <th className="px-2 hidden sm:table-cell">Tanggal Verifikasi</th>
                <th className="px-2 hidden md:table-cell">Diverifikasi Oleh</th>
                ...
                >
                  <td className="py-3 px-2 font-medium">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-gray-500 md:hidden">{record.medicalRecordNumber}</span>
                    </div>
                  </td>
                  <td className="px-2 hidden md:table-cell">{record.medicalRecordNumber}</td>
                  <td className="px-2 hidden sm:table-cell">{record.verificationDate}</td>
                  <td className="px-2 hidden md:table-cell">{record.verifiedBy}</td>
                  <td className="px-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white/70 dark:bg-zinc-900/60 border border-gray-200 dark:border-zinc-800 backdrop-blur-md shadow-xl rounded-2xl p-4 md:p-6">
          <h2 className="text-lg md:text-xl font-bold mb-4">Statistik Verifikasi</h2>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-green-50 dark:bg-green-900/30 p-3 rounded-lg">
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">Terverifikasi</p>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-lg">
              ...
              <p className="text-lg md:text-2xl font-bold">{records.length}</p>
            </div>
          </div>
        </div>
        
        <div className="bg-white/70 dark:bg-zinc-900/60 border border-gray-200 dark:border-zinc-800 backdrop-blur-md shadow-xl rounded-2xl p-4 md:p-6">
          <h2 className="text-lg md:text-xl font-bold mb-4">Rekap Verifikasi</h2>
