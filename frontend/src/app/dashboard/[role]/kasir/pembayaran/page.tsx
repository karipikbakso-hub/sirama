'use client'

import { useState } from 'react'
import { FaMoneyBillWave, FaSearch, FaCheckCircle, FaPrint } from 'react-icons/fa'

type Payment = {
  id: number
  patientName: string
  medicalRecordNumber: string
  paymentDate: string
  amount: number
  paymentMethod: 'cash' | 'debit' | 'credit' | 'insurance'
  status: 'success' | 'pending' | 'failed'
}

const initialData: Payment[] = [
  {
    id: 1,
    patientName: 'Budi Santoso',
    medicalRecordNumber: 'MR-2025-001',
    paymentDate: '2025-11-05',
    amount: 150000,
    paymentMethod: 'cash',
    status: 'success'
  },
  {
    id: 2,
    patientName: 'Dewi Lestari',
    medicalRecordNumber: 'MR-2025-002',
    paymentDate: '2025-11-04',
    amount: 125000,
    paymentMethod: 'debit',
    status: 'success'
  },
  {
    id: 3,
    patientName: 'Andi Prasetyo',
    medicalRecordNumber: 'MR-2025-003',
    paymentDate: '2025-11-03',
    amount: 125000,
    paymentMethod: 'credit',
    status: 'success'
  },
  {
    id: 4,
    patientName: 'Siti Rahayu',
    medicalRecordNumber: 'MR-2025-004',
    paymentDate: '2025-11-02',
    amount: 250000,
    paymentMethod: 'insurance',
    status: 'pending'
  }
]

export default function PembayaranPage() {
  const [payments] = useState(initialData)
  const [searchTerm, setSearchTerm] = useState('')

  const filteredPayments = payments.filter(payment =>
    payment.patientName.toLowerCase().includes(searchTerm.toLowerCase()) ||
    payment.medicalRecordNumber.toLowerCase().includes(searchTerm.toLowerCase())
  )

  const getStatusColor = (status: string) => {
    switch (status) {
      case 'success': return 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'
      case 'pending': return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200'
      case 'failed': return 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'
      default: return 'bg-gray-100 text-gray-800 dark:bg-gray-900 dark:text-gray-200'
    }
  }

  const getStatusText = (status: string) => {
    switch (status) {
      case 'success': return 'Berhasil'
      case 'pending': return 'Menunggu'
      case 'failed': return 'Gagal'
      default: return status
    }
  }

  const getPaymentMethodIcon = (method: string) => {
    switch (method) {
      case 'cash': return '💵'
      case 'debit': return '💳'
      case 'credit': return '💳'
      case 'insurance': return '🏥'
      default: return '💰'
    }
  }

  const getPaymentMethodText = (method: string) => {
    switch (method) {
      case 'cash': return 'Tunai'
      case 'debit': return 'Debit'
      case 'credit': return 'Kredit'
      case 'insurance': return 'Asuransi'
      default: return method
    }
  }

  const formatCurrency = (amount: number) => {
    return new Intl.NumberFormat('id-ID', {
      style: 'currency',
      currency: 'IDR',
      minimumFractionDigits: 0
    }).format(amount)
  }

  return (
    <div className="space-y-6">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800 dark:text-white flex items-center gap-3">
          <FaMoneyBillWave className="text-blue-500" />
          <span>Pembayaran</span>
        </h1>
        <p className="text-gray-600 dark:text-gray-400 mt-1">
          Kelola pembayaran dan transaksi pasien
        </p>
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 border border-gray-200 dark:border-gray-700">
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
              <FaSearch className="text-gray-400" />
            </div>
            <input
              type="text"
              placeholder="Cari pembayaran..."
              className="w-full pl-10 px-4 py-2 rounded-lg border border-gray-300 dark:border-zinc-700 bg-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-zinc-700 text-left">
                <th className="py-3 px-2">Nama Pasien</th>
                <th className="px-2 hidden md:table-cell">No. Rekam Medis</th>
                <th className="px-2 hidden sm:table-cell">Tanggal Pembayaran</th>
                <th className="px-2">Jumlah</th>
                <th className="px-2 hidden md:table-cell">Metode</th>
                <th className="px-2">Status</th>
                <th className="text-right px-2">Aksi</th>
              </tr>
            </thead>
            <tbody>
              {filteredPayments.map((payment) => (
                <tr
                  key={payment.id}
                  className="border-b border-gray-200 dark:border-zinc-800 hover:bg-indigo-500/10 dark:hover:bg-indigo-400/10 transition"
                >
                  <td className="py-3 px-2 font-medium">
                    <div className="flex flex-col">
                      <span>{payment.patientName}</span>
                      <span className="text-xs text-gray-500 md:hidden">{payment.medicalRecordNumber}</span>
                    </div>
                  </td>
                  <td className="px-2 hidden md:table-cell">{payment.medicalRecordNumber}</td>
                  <td className="px-2 hidden sm:table-cell">{payment.paymentDate}</td>
                  <td className="px-2 font-medium">{formatCurrency(payment.amount)}</td>
                  <td className="px-2 hidden md:table-cell">
                    <span className="flex items-center gap-1">
                      <span>{getPaymentMethodIcon(payment.paymentMethod)}</span>
                      <span>{getPaymentMethodText(payment.paymentMethod)}</span>
                    </span>
                  </td>
                  <td className="px-2">
                    <span className={`px-2 py-1 rounded-full text-xs ${getStatusColor(payment.status)}`}>
                      {getStatusText(payment.status)}
                    </span>
                  </td>
                  <td className="text-right px-2">
                    <div className="flex justify-end gap-1">
                      <button className="p-2 border border-gray-300 dark:border-zinc-700 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-800 transition">
                        <FaPrint />
                      </button>
                      <button className="p-2 rounded-lg bg-green-500 hover:bg-green-600 text-white transition">
                        <FaCheckCircle />
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>

        {filteredPayments.length === 0 && (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            <FaMoneyBillWave className="mx-auto text-4xl mb-2" />
            <p>Tidak ada pembayaran yang ditemukan</p>
          </div>
        )}
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">
            Statistik Pembayaran
          </h3>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-green-50 dark:bg-green-900/30 p-3 rounded-lg">
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">Berhasil</p>
              <p className="text-lg md:text-2xl font-bold text-gray-800 dark:text-white">
                {payments.filter(p => p.status === 'success').length}
              </p>
            </div>
            <div className="bg-yellow-50 dark:bg-yellow-900/30 p-3 rounded-lg">
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">Menunggu</p>
              <p className="text-lg md:text-2xl font-bold text-gray-800 dark:text-white">
                {payments.filter(p => p.status === 'pending').length}
              </p>
            </div>
            <div className="bg-red-50 dark:bg-red-900/30 p-3 rounded-lg">
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">Gagal</p>
              <p className="text-lg md:text-2xl font-bold text-gray-800 dark:text-white">
                {payments.filter(p => p.status === 'failed').length}
              </p>
            </div>
            <div className="bg-blue-50 dark:bg-blue-900/30 p-3 rounded-lg">
              <p className="text-xs md:text-sm text-gray-600 dark:text-gray-300">Total Pembayaran</p>
              <p className="text-lg md:text-2xl font-bold text-gray-800 dark:text-white">{payments.length}</p>
            </div>
          </div>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">
            Rekap Pendapatan
          </h3>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-800 dark:text-white">Hari Ini</span>
              <span className="font-bold text-gray-800 dark:text-white">{formatCurrency(550000)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-800 dark:text-white">Minggu Ini</span>
              <span className="font-bold text-gray-800 dark:text-white">{formatCurrency(3250000)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-800 dark:text-white">Bulan Ini</span>
              <span className="font-bold text-gray-800 dark:text-white">{formatCurrency(15750000)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-800 dark:text-white">Metode Pembayaran Terpopuler</span>
              <span className="font-bold text-gray-800 dark:text-white">Tunai</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
