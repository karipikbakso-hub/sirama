...
const roles = [
  ...
  { label: 'Rawat Inap', value: 'perawat' },
...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    ...
    localStorage.setItem('sirama-role', selectedRole)
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-white px-4 transition-colors duration-500">
      <div
        className={clsx(
          'w-full max-w-3xl p-8 rounded-2xl shadow-xl border transition-all duration-700',
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 justify-center">
          {roles.map(({ label, value }) => (
            <button
              key={value}
              onClick={() => setSelectedRole(value)}
              className={clsx(
                'min-w-[100px] px-4 py-2 rounded-lg border text-sm font-medium transition shadow-sm',
                selectedRole === value
