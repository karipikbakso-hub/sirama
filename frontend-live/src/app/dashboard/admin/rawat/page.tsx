'use client'

import { useState } from 'react'

const dummyInap = Array.from({ length: 42 }, (_, i) => ({
  id: i + 1,
  nama: ['Andi', 'Budi', 'Citra', 'Dewi', 'Eka'][i % 5],
  dokter: ['dr. Sari', 'dr. Rudi'][i % 2],
  kamar: ['Kelas 1', 'Kelas 2', 'VIP'][i % 3],
  status: ['Aktif', 'Pulang', 'Pindah'][i % 3],
  masuk: `2025-10-${(i % 30) + 1}`,
  durasi: `${2 + i} hari`,
}))

const statusColor = {
  Aktif: 'bg-blue-100 text-blue-700 dark:bg-blue-800 dark:text-blue-200',
  Pulang: 'bg-green-100 text-green-700 dark:bg-green-800 dark:text-green-200',
  Pindah: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-800 dark:text-yellow-200',
}

export default function RawatInapPage() {
  const [filterKamar, setFilterKamar] = useState('Semua')
  const [filterStatus, setFilterStatus] = useState('Semua')
  const [filterTanggal, setFilterTanggal] = useState('')
  const [search, setSearch] = useState('')
  const [page, setPage] = useState(1)
  const [perPage, setPerPage] = useState(10)

  const inapFiltered = dummyInap.filter(
    (p) =>
      (filterKamar === 'Semua' || p.kamar === filterKamar) &&
      (filterStatus === 'Semua' || p.status === filterStatus) &&
      (filterTanggal === '' || p.masuk === filterTanggal) &&
      (search === '' ||
        p.nama.toLowerCase().includes(search.toLowerCase()) ||
        p.dokter.toLowerCase().includes(search.toLowerCase()) ||
        p.kamar.toLowerCase().includes(search.toLowerCase()))
  )

  const totalPages = Math.ceil(inapFiltered.length / perPage)
  const paginated = inapFiltered.slice((page - 1) * perPage, page * perPage)

  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-gray-800 dark:text-white">🛏️ Rawat Inap</h2>

      {/* Filter */}
      <div className="flex flex-wrap items-center gap-4">
        <input
          type="text"
          placeholder="🔍 Cari pasien, dokter, kamar..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="px-3 py-2 w-full md:w-64 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200"
        />
        <input
          type="date"
          value={filterTanggal}
          onChange={(e) => setFilterTanggal(e.target.value)}
          className="px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200"
        />
        <select
          value={filterKamar}
          onChange={(e) => setFilterKamar(e.target.value)}
          className="appearance-none px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200"
        >
          <option>Semua</option>
          <option>Kelas 1</option>
          <option>Kelas 2</option>
          <option>VIP</option>
        </select>
        <select
          value={filterStatus}
          onChange={(e) => setFilterStatus(e.target.value)}
          className="appearance-none px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200"
        >
          <option>Semua</option>
          <option>Aktif</option>
          <option>Pulang</option>
          <option>Pindah</option>
        </select>
        <select
          value={perPage}
          onChange={(e) => { setPerPage(Number(e.target.value)); setPage(1) }}
          className="appearance-none px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-200"
        >
          <option value={10}>10 / halaman</option>
          <option value={25}>25 / halaman</option>
          <option value={50}>50 / halaman</option>
        </select>
      </div>

      {/* Pagination Atas */}
      <PaginationBar
        page={page}
        totalPages={totalPages}
        totalItems={inapFiltered.length}
        perPage={perPage}
        onPageChange={setPage}
      />

      {/* Tabel */}
      <div className="overflow-x-auto rounded border border-gray-200 dark:border-gray-700 shadow">
        <table className="min-w-full text-sm text-left bg-white dark:bg-gray-800">
          <thead className="bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-200">
            <tr>
              <th className="p-3 border-b">#</th>
              <th className="p-3 border-b">Nama</th>
              <th className="p-3 border-b">Dokter</th>
              <th className="p-3 border-b">Kamar</th>
              <th className="p-3 border-b">Masuk</th>
              <th className="p-3 border-b">Durasi</th>
              <th className="p-3 border-b">Status</th>
              <th className="p-3 border-b">Aksi</th>
            </tr>
          </thead>
          <tbody>
            {paginated.map((p, i) => (
              <tr key={p.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                <td className="p-3 text-gray-700 dark:text-gray-300">{(page - 1) * perPage + i + 1}</td>
                <td className="p-3 text-gray-700 dark:text-gray-300">{p.nama}</td>
                <td className="p-3 text-gray-700 dark:text-gray-300">{p.dokter}</td>
                <td className="p-3 text-gray-700 dark:text-gray-300">{p.kamar}</td>
                <td className="p-3 text-gray-700 dark:text-gray-300">{p.masuk}</td>
                <td className="p-3 text-gray-700 dark:text-gray-300">{p.durasi}</td>
                <td>
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${statusColor[p.status]}`}>
                    {p.status}
                  </span>
                </td>
                <td className="p-3 space-x-2">
                <button className="px-3 py-1 text-xs rounded bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 hover:scale-105 transition">
                    📄 Detail
                </button>
                <button className="px-3 py-1 text-xs rounded bg-yellow-100 dark:bg-yellow-900 text-yellow-600 dark:text-yellow-300 hover:scale-105 transition">
                    🔄 Pindah
                </button>
                <button className="px-3 py-1 text-xs rounded bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-300 hover:scale-105 transition">
                    ✅ Pulangkan
                </button>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      {/* Pagination Bawah */}
      <PaginationBar
        page={page}
        totalPages={totalPages}
        totalItems={inapFiltered.length}
        perPage={perPage}
        onPageChange={setPage}
      />
    </div>
  )
}

function PaginationBar({
  page,
  totalPages,
  totalItems,
  perPage,
  onPageChange,
}: {
  page: number
  totalPages: number
  totalItems: number
  perPage: number
  onPageChange: (p: number) => void
}) {
  return (
    <div className="flex justify-between items-center text-sm text-gray-600 dark:text-gray-300 py-2">
      <span>
        Menampilkan {(page - 1) * perPage + 1}–{Math.min(page * perPage, totalItems)} dari {totalItems} pasien
      </span>
      <div className="space-x-2">
        <button
          onClick={() => onPageChange(Math.max(1, page - 1))}
          disabled={page === 1}
          className="px-3 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50"
        >
          ← Sebelumnya
        </button>
        <button
          onClick={() => onPageChange(Math.min(totalPages, page + 1))}
          disabled={page === totalPages}
          className="px-3 py-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50"
        >
          Selanjutnya →
        </button>
      </div>
    </div>
  )

  
}