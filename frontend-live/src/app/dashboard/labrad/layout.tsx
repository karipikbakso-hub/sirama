...
export default function LabRadLayout({ children }: { children: React.ReactNode }) {
  ...
  const items = [
    { label: 'Laboratorium', icon: MdScience, href: '/dashboard/labrad/lab' },
    { label: 'Radiologi', icon: MdCameraAlt, href: '/dashboard/labrad/rad' },
    { label: 'Upload Hasil', icon: MdUploadFile, href: '/dashboard/labrad/upload' },
    { label: 'Cetak Hasil', icon: MdPrint, href: '/dashboard/labrad/cetak' },
  ]
    ...
    <>
      <div className="flex min-h-screen bg-background text-foreground">
        ...
        <aside className="hidden md:flex flex-col w-64 h-screen bg-card border-r border-border px-6 py-8 space-y-6 text-sm">
          <Link href="/dashboard/labrad" className="text-center text-4xl font-bold tracking-wide select-none">
            SIRAMA
          </Link>
