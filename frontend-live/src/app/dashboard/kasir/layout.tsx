...
export default function KasirLayout({ children }: { children: React.ReactNode }) {
  ...
  const items = [
    { label: 'Billing & Pembayaran', icon: MdPayment, href: '/dashboard/kasir/billing' },
    { label: 'Kwitansi', icon: MdReceiptLong, href: '/dashboard/kasir/kwitansi' },
    { label: 'Pembayaran', icon: MdAttachMoney, href: '/dashboard/kasir/pembayaran' },
    { label: 'Cetak Tagihan', icon: MdPrint, href: '/dashboard/kasir/tagihan' },
  ]
    ...
    <>
      <div className="flex min-h-screen bg-background text-foreground">
        ...
        <aside className="hidden md:flex flex-col w-64 h-screen bg-card border-r border-border px-6 py-8 space-y-6 text-sm">
          <Link href="/dashboard/kasir" className="text-center text-4xl font-bold tracking-wide select-none">
            SIRAMA
          </Link>

          <div>
            <div className="px-3 py-2 text-xs uppercase tracking-wider text-muted-foreground">Modul Kasir</div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <header className="sticky top-0 z-30 bg-card border-b px-6 py-4 flex justify-between items-center shadow-sm">
            <h1 className="text-xl font-semibold tracking-wide">Dashboard Kasir</h1>
